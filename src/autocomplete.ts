...
function getFileCompletions(partial: string, cwd: string, type: "file" | "folder"): string[] {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (!item.name.toLowerCase().startsWith(prefix.toLowerCase())) continue;
...
function getImageCompletions(partial: string, cwd: string): string[] {
  const completions: string[] = [];
...
export function createReadlineWithCompletion(cwd: string): readline.Interface {
...
export function showCompletionMenu(completions: string[]): void {

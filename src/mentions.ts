...
export interface MentionResult {
  type: "file" | "folder" | "web" | "symbol" | "git";
  value: string;
  content?: string;
}

// Parse @ mentions from message
export function parseMentions(message: string, cwd: string): { cleanMessage: string; mentions: MentionResult[] } {
...
function getFolderContent(folderPath: string, cwd: string, depth = 0, maxDepth = 2): string {
  ...
  for (const item of items) {
    if (item.name.startsWith(".") || IGNORE.includes(item.name)) continue;
...
export async function buildMentionContext(mentions: MentionResult[], cwd: string): Promise<string> {
...
// Get git context
async function getGitContext(cwd: string, type: string): Promise<string> {
...
export function formatMentions(mentions: MentionResult[]): string {
  if (mentions.length === 0) return "";

 ...
 */
export function saveEmbeddingIndex(rootDir: string): boolean {
  ...
  try {
    ...
    const serialized: SerializedEmbeddingIndex = {
 ...
 */
export function loadEmbeddingIndex(rootDir: string): EmbeddingIndex | null {
  const cachePath = getCachePath(rootDir);
  ...
  try {
    ...
    const serialized: SerializedEmbeddingIndex = JSON.parse(content);
...
export async function semanticSearch(
  ...
  query: string,
  topK: number = 10
): Promise<{ chunk: EmbeddingChunk; score: number }[]> {
  ...
  const results: { chunk: EmbeddingChunk; score: number }[] = [];
 ...
 */
export function getEmbeddingIndex(): EmbeddingIndex | null {
  return embeddingIndex;
}

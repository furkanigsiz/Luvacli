 ...
 */
function parseFrontmatter(content: string): { frontmatter: SteeringFrontmatter; body: string } {
  ...
  try {
    const frontmatter = yaml.parse(match[1]) as SteeringFrontmatter;
 ...
 */
export function discoverSteeringFiles(cwd: string): SteeringFile[] {
 ...
 */
export function getActiveSteeringFiles(
  cwd: string,
  activeFiles: string[] = [],
  manualIncludes: string[] = []
): SteeringFile[] {
  const allFiles = discoverSteeringFiles(cwd);
  const active: SteeringFile[] = [];
 ...
 */
export function buildSteeringContext(files: SteeringFile[]): string {
 ...
 */
export function listSteeringFiles(cwd: string): string {
...
export function createSteeringFile(
  ...
  name: string,
  content: string,
  options: { inclusion?: string; fileMatchPattern?: string; description?: string } = {}
): string {
  const steeringDir = path.join(cwd, STEERING_DIR);
 ...
 */
export function parseSteeringMentions(message: string): { cleanMessage: string; mentions: string[] } {

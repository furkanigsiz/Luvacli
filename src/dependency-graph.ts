...
export interface DependencyNode {
  ...
  importedBy: string[];   // Bu dosyayı import eden dosyalar
  depth: number;          // Root'tan uzaklık
}
 ...
 */
function resolveImportPath(importPath: string, fromDir: string, rootDir: string): string | null {
...
export function getDependencies(
  ...
  function traverse(currentFile: string, depth: number) {
 ...
 */
export function formatDependencyInfo(
  graph: DependencyGraph,
  file: string
): string {
  const node = graph.nodes.get(file);
  if (!node) return `❌ Dosya bulunamadı: ${file}`;

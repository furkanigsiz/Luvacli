...
// Re-export EmbeddingChunk type for use in other modules
 ...
 */
function parseWithRegex(filePath: string, content: string): EmbeddingChunk[] {
  ...
  const patterns = [
    ...
    { regex: /^(?:export\s+)?const\s+(\w+)\s*=\s*(?:async\s+)?\(/m, type: "function" as const },
    { regex: /^(?:export\s+)?(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?(?:\([^)]*\)|[^=])\s*=>/m, type: "function" as const },
    { regex: /^(?:export\s+)?(?:abstract\s+)?class\s+(\w+)/m, type: "class" as const },
    { regex: /^(?:export\s+)?interface\s+(\w+)/m, type: "interface" as const },
    { regex: /^(?:export\s+)?type\s+(\w+)/m, type: "interface" as const },
    { regex: /^(?:async\s+)?def\s+(\w+)/m, type: "function" as const },
 ...
 */
export async function parseFileIntoChunks(filePath: string): Promise<EmbeddingChunk[]> {
 ...
 */
export function parseFileIntoChunksSync(filePath: string): EmbeddingChunk[] {
  if (!fs.existsSync(filePath)) return [];
 ...
 */
export async function chunkCodebase(rootDir: string): Promise<EmbeddingChunk[]> {
 ...
 * Get chunk summary
 */
export function getChunkSummary(chunks: EmbeddingChunk[]): string {

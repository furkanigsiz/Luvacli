...
export function generateTestContent(sourceFile, sourceContent, framework) {
...
function analyzeExport(name, content) {
...
function generateTestCase(name, info, framework) {
...
`;
    ...
    if (info.type === "function") {
        const paramPlaceholders = info.params.map((p, i) => {
            ...
            if (p.includes("name") || p.includes("str"))
                return `"test"`;
            if (p.includes("num") || p.includes("count") || p.includes("id"))
                return `1`;
            if (p.includes("arr") || p.includes("list"))
                return `[]`;
            if (p.includes("obj") || p.includes("data"))
                return `{}`;
            if (p.includes("flag") || p.includes("is") || p.includes("has"))
                return `true`;
            return `undefined /* ${p} */`;
        }).join(", ");
        return `
...
function generatePlaceholderTest(fileName, framework) {
    return `
describe("${fileName}", () => {
...
export function getTestFilePath(sourceFile) {
...
export function generateTestForFile(sourceFile, cwd) {
...
export function getInstallCommand(framework) {
    switch (framework) {
        case "vitest":
...
// Get run command
export function getRunCommand(framework, testFile) {
...
export function formatTestGenResult(testPath, framework) {
    return `✅ Test dosyası oluşturuldu: ${testPath}

Framework: ${framework}
Çalıştırmak için: ${getRunCommand(framework, testPath)}`;
}

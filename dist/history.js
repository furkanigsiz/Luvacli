...
export function takeSnapshot(filePath, operation, description) {
...
export function undoLastChange(filePath) {
    if (filePath) {
        // Undo specific file
        const resolved = path.resolve(filePath);
        const history = fileHistory.get(resolved);
...
export function getFileHistory(filePath) {
    ...
    for (const snap of recent) {
        ...
        if (snap.description)
            output += `  📝 ${snap.description}\n`;
        output += "\n";
    }
    return output;
}
// Get recent changes (global)
export function getRecentChanges(count = 10) {
    if (changeLog.length === 0) {

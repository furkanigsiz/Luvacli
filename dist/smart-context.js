...
import { chunkCodebase, getChunkSummary } from "./chunker.js";
import { indexWithEmbeddings, semanticSearch, getEmbeddingIndex, loadEmbeddingIndex } from "./embeddings.js";
import { buildDependencyGraph, getRelatedFiles, getGraphSummary } from "./dependency-graph.js";
import { buildSmartContext, TOKEN_BUDGET } from "./context-optimizer.js";
let smartIndex = null;
/**
 * Initialize smart context index
 */
export async function initSmartContext(genAI, rootDir, forceReindex = false) {
 ...
 */
export async function getSmartContext(genAI, query, options = {}) {
    ...
    if (options.activeFiles) {
        for (const file of options.activeFiles) {
            if (options.mentionedFiles?.includes(file))
                continue;
            const fullPath = path.resolve(smartIndex?.rootDir || process.cwd(), file);
            if (fs.existsSync(fullPath)) {
                const content = fs.readFileSync(fullPath, "utf-8");
    ...
    if (smartIndex && getEmbeddingIndex()) {
        ...
        for (const { chunk, score } of semanticResults) {
            ...
            if (contextItems.some(c => c.file === chunk.file))
                continue;
    ...
    if (smartIndex && (options.activeFiles?.length || options.mentionedFiles?.length)) {
        ...
        for (const file of targetFiles) {
            ...
            for (const dep of [...related.dependencies, ...related.dependents].slice(0, 3)) {
                if (addedDeps.has(dep) || contextItems.some(c => c.file === dep))
                    continue;
                addedDeps.add(dep);
 ...
 */
export function getSmartIndex() {
    return smartIndex;
}
/**
 * Check if smart index is ready
 */
export function isSmartIndexReady() {
    return smartIndex !== null && getEmbeddingIndex() !== null;
}
/**
 * Format smart context info
 */
export function formatSmartContextInfo() {
    if (!smartIndex) {

 ...
 */
export async function runAgentMode(model, goal, codebaseContext, config = {}) {
 ...
 */
async function createPlan(model, goal, codebaseContext) {
 ...
 */
async function executeStep(model, step, plan, codebaseContext) {
 ...
 */
async function checkAndFix(model, codebaseContext, skipDependencyErrors = true) {
    ...
    try {
        ...
        const filteredDiagnostics = result.diagnostics.filter(d => {
            ...
            if (d.file.includes("node_modules"))
                return false;
            // Skip JSX/tsx config errors (Vite handles this at runtime)
            if (d.message.includes("--jsx") || d.message.includes("jsx flag"))
                return false;
            // Skip module resolution errors (usually dependency issues)
            if (skipDependencyErrors) {
                if (d.message.includes("Cannot find module"))
                    return false;
                if (d.message.includes("Could not find a declaration file"))
                    return false;
                if (d.message.includes("has no exported member"))
                    return false;
                if (d.message.includes("Cannot find namespace"))
                    return false;
                if (d.message.includes("esModuleInterop"))
                    return false;
            }
            return true;
        });
...
function printSummary(plan) {
    ...
    if (failed > 0) {
        ...
        for (const step of plan.steps.filter(s => s.status === "failed")) {
            ...
            if (step.error)
                console.log(`    ${c.dim}${step.error}${c.reset}`);
        }
    }
}
/**
 ...
 */
export async function runAgentFromSpec(model, spec, codebaseContext, config = {}) {
    // Convert spec tasks to agent steps
    const steps = spec.tasks

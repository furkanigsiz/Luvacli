...
export function buildSmartContext(items, maxTokens = TOKEN_BUDGET.total) {
 ...
 */
export function optimizeHistory(history, maxTokens = 50000) {
    if (history.length === 0)
        return history;
 ...
 */
export function summarizeOldHistory(history, keepLast = 10) {
 ...
 */
export function deduplicateContext(history) {
 ...
 */
export function truncateFileContent(content, maxLines = 100) {
 ...
 */
export function getContextStats(history, systemPrompt) {
    let historyTokens = 0;
    for (const msg of history) {
        const text = getTextFromParts(msg.parts || []);

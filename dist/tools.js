...
import { indexCodebase, searchSymbols, findReferences, getFileContext } from "./codebase.js";
...
import { multiFileEdit, rollbackTransaction, listTransactions } from "./multifile.js";
import { takeSnapshot, restoreSnapshot, undoLastChange, getFileHistory, getRecentChanges } from "./history.js";
import { startProcess, stopProcess, getProcessOutput, formatProcessList } from "./process.js";
...
import { checkCommand, checkFileWrite, checkFileDelete } from "./security.js";
const execAsync = promisify(exec);
// Codebase index cache
let codebaseCache = null;
...
function setCache(key, result) {
    ...
    if (result.startsWith("❌") || result.length > 50000)
        return;
    ...
    try {
        ...
        switch (name) {
            ...
            case "write_file":
                ...
                break;
            case "append_file":
                result = appendFile(args.path, args.content);
                clearToolCache();
                break;
            case "list_directory":
                ...
                break;
            case "run_command":
                result = await runCommand(args.command, args.cwd);
                break;
            case "create_directory":
                result = createDirectory(args.path);
                break;
            case "delete_file":
                ...
                break;
            case "search_files":
                result = searchFiles(args.directory, args.pattern, args.extension);
                break;
            case "edit_file":
                result = editFile(args.path, args.old_text, args.new_text);
                clearToolCache();
                break;
            case "git_status":
                result = await gitStatus(args.directory);
                break;
            case "git_diff":
                result = await gitDiff(args.directory, args.file);
                break;
            case "git_commit":
                result = await gitCommit(args.directory, args.message, args.add_all);
                break;
            case "web_search":
                result = await webSearch(args.query);
                break;
            case "get_file_structure":
                result = getFileStructure(args.directory, args.max_depth || 3);
                break;
            // Codebase tools
            case "index_codebase":
                result = await indexCodebaseCmd(args.directory);
                break;
            case "search_symbols":
                result = await searchSymbolsCmd(args.directory, args.query);
                break;
            case "find_references":
                result = await findReferencesCmd(args.directory, args.symbol);
                break;
            case "get_file_context":
                result = await getFileContextCmd(args.directory, args.file);
                break;
            // Diagnostics tools
            case "get_diagnostics":
                result = await getDiagnosticsCmd(args.paths);
                break;
            case "run_diagnostics":
                result = await runDiagnosticsCmd(args.directory);
                break;
            case "check_file_syntax":
                result = checkFileSyntaxCmd(args.path);
                break;
            // Context tools
            case "get_relevant_context":
                result = getRelevantContextCmd(args.directory, args.query, args.current_file);
                break;
            case "pin_file":
                result = pinFileCmd(args.path);
                break;
            case "unpin_file":
                result = unpinFileCmd(args.path);
                break;
            case "context_status":
                result = contextStatusCmd();
                break;
            // Multi-file tools
            case "multi_file_edit":
                result = multiFileEditCmd(args.edits);
                clearToolCache();
                break;
            case "rollback_transaction":
                result = rollbackTransactionCmd(args.transaction_id);
                clearToolCache();
                break;
            ...
            // Undo/Restore tools
            case "undo":
                result = undoCmd(args.file);
                clearToolCache();
                break;
            case "restore":
                result = restoreCmd(args.snapshot_id);
                clearToolCache();
                break;
            case "history":
                result = historyCmd(args.file, args.count);
                break;
            // Background Process tools
            case "start_process":
                result = startProcessCmd(args.command, args.cwd);
                break;
            case "stop_process":
                result = stopProcessCmd(args.process_id);
                break;
            case "get_process_output":
                result = getProcessOutputCmd(args.process_id, args.lines);
                break;
            case "list_processes":
                result = listProcessesCmd();
                break;
            // Steering tools
            case "list_steering":
                result = listSteeringCmd(args.directory);
                break;
            case "create_steering":
                result = createSteeringCmd(args.directory, args.name, args.content, args.inclusion, args.file_match_pattern, args.description);
                break;
            case "read_steering":
                result = readSteeringCmd(args.directory, args.name);
                break;
...
function listDirectory(dirPath) {
    ...
    if (!fs.existsSync(resolved))
        return `Klasör bulunamadı: ${resolved}`;
...
async function runCommand(command, cwd) {
    ...
    const options = { timeout: 60000, shell: process.platform === "win32" ? "powershell.exe" : "/bin/bash" };
    if (cwd)
        options.cwd = resolvePath(cwd);
...
function deleteFile(filePath) {
    ...
    const resolved = resolvePath(filePath);
    if (!fs.existsSync(resolved))
        return `Dosya/klasör bulunamadı: ${resolved}`;
...
function searchFiles(directory, pattern, extension) {
...
function editFile(filePath, oldText, newText) {
    ...
    if (!content.includes(oldText))
        return `❌ Metin bulunamadı dosyada.`;
...
async function gitDiff(directory, file) {
    const resolved = resolvePath(directory);
    try {
...
async function gitCommit(directory, message, addAll) {
    const resolved = resolvePath(directory);
    try {
        if (addAll)
...
async function webSearch(query) {
    try {
        ...
        if (data.RelatedTopics?.length > 0) {
            result += "\n📌 İlgili:\n";
            data.RelatedTopics.slice(0, 5).forEach((t) => { if (t.Text)
                result += `• ${t.Text.slice(0, 100)}...\n`; });
        }
...
function getFileStructure(directory, maxDepth) {
    const resolved = resolvePath(directory);
    const lines = [`📂 ${resolved}`];
    function traverse(dir, prefix, depth) {
        if (depth > maxDepth)
            return;
        const items = fs.readdirSync(dir, { withFileTypes: true })
        ...
        items.forEach((item, idx) => {
            ...
            if (item.isDirectory())
                traverse(path.join(dir, item.name), prefix + (isLast ? "    " : "│   "), depth + 1);
        });
    }
    traverse(resolved, "", 1);
    return lines.join("\n");
...
async function getOrCreateIndex(directory) {
    const resolved = resolvePath(directory);
    // Check cache
...
async function indexCodebaseCmd(directory) {
    const index = await getOrCreateIndex(directory);
    return index.summary;
}
async function searchSymbolsCmd(directory, query) {
    const index = await getOrCreateIndex(directory);
...
async function findReferencesCmd(directory, symbol) {
    const index = await getOrCreateIndex(directory);
...
async function getFileContextCmd(directory, file) {
    const index = await getOrCreateIndex(directory);
    return getFileContext(index, file);
}
// ============ Diagnostics Functions ============
async function getDiagnosticsCmd(paths) {
...
async function runDiagnosticsCmd(directory) {
    const resolved = resolvePath(directory);
    const result = await runAllDiagnostics(resolved);
    return formatDiagnostics(result);
}
function checkFileSyntaxCmd(filePath) {
    const resolved = resolvePath(filePath);
    if (!fs.existsSync(resolved)) {
        return `❌ Dosya bulunamadı: ${resolved}`;
    }
...
function getRelevantContextCmd(directory, query, currentFile) {
    const resolved = resolvePath(directory);
...
function rollbackTransactionCmd(transactionId) {
    const result = rollbackTransaction(transactionId);
    return result.message;
}
function listTransactionsCmd() {
    return listTransactions();
}
// ============ Undo/Restore Functions ============
function undoCmd(filePath) {
    const result = undoLastChange(filePath);
    return result.message;
}
function restoreCmd(snapshotId) {
    const result = restoreSnapshot(snapshotId);
    return result.message;
}
function historyCmd(filePath, count) {
    if (filePath) {
...
function startProcessCmd(command, cwd) {
    const workDir = cwd ? resolvePath(cwd) : process.cwd();
    const result = startProcess(command, workDir);
    return result.message;
}
function stopProcessCmd(processId) {
    const result = stopProcess(processId);
    return result.message;
}
function getProcessOutputCmd(processId, lines) {
    const result = getProcessOutput(processId, lines);
    return `📋 Process ${processId} [${result.status}]:\n\n${result.output}`;
}
function listProcessesCmd() {
    return formatProcessList();
}
// ============ Steering Functions ============
function listSteeringCmd(directory) {
    const resolved = resolvePath(directory);
    return listSteeringFiles(resolved);
}
function createSteeringCmd(directory, name, content, inclusion, fileMatchPattern, description) {
    const resolved = resolvePath(directory);
...
function readSteeringCmd(directory, name) {
    const resolved = resolvePath(directory);
    ...
    if (file.fileMatchPattern)
        output += `🎯 Pattern: ${file.fileMatchPattern}\n`;
    output += `\n${file.content}`;
    return output;
}

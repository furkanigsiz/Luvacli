...
export function selectRelevantFiles(rootDir, query, currentFile) {
...
function scanAndScore(dir, root, files, keywords, query, currentFile) {
...
function scoreFile(relativePath, content, keywords, query, currentFile) {
    let relevance = 0;
    const reasons = [];
    ...
    for (const kw of keywords) {
        ...
        if (matches > 0) {
            ...
            if (!reasons.includes(`content: ${kw}`))
...
function fitToTokenBudget(files, maxTokens) {
    const result = [];
    let totalChars = 0;
...
export function buildContextString(files) {
    if (files.length === 0)
        return "";
...
// Get context summary
export function getContextSummary(files) {
